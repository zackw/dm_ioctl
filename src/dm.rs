...
impl DM {
    ...
    fn hdr_set_uuid(hdr: &mut Struct_dm_ioctl, uuid: &DmUuid) -> DmResult<()> {
        let _ = uuid
            .as_bytes()
    ...
    pub fn target_msg(
        ...
        let mut data_in = unsafe {
            let ptr = &msg_struct as *const Struct_dm_target_msg as *mut u8;
            slice::from_raw_parts(ptr, size_of::<Struct_dm_target_msg>())
                .to_vec()
        };
